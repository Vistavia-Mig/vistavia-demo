...
// EDITA ESTO con tu WhatsApp y (opcional) Yape
const WHATSAPP_LINK =
  "https://wa.me/51999999999?text=Hola%20soy%20agente%20y%20quiero%20acceso%20a%20Vistavia";
const YAPE_LINK = "#";

// Isotipo Vistavia (V-pin)
const VistaviaMark = ({ size = 36 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 96 96" aria-label="Vistavia mark">
    <path
      d="M18 24h18c2.5 0 4.9 1.2 6.3 3.3L48 41l5.7-13.7c1.4-2.1 3.8-3.3 6.3-3.3H78c2.7 0 4.4 2.8 3.2 5.2L52.8 78.3c-2.2 4.2-7.9 4.2-10.1 0L14.8 29.2C13.6 26.8 15.3 24 18 24z"
      fill="#4F46E5"
    />
...
const DUMMY_LISTINGS: Listing[] = [
  ...
  { id: 3, title: "San Isidro — Oficina 80 m²", priceUSD: 210000, district: "San Isidro", type: "Oficina", m2: 80, status: "Exclusivo", coords: [-12.1, -77.04] },
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {route === "explore" && (
        <Explore
          ...
          onGoPricing={() => setRoute("pricing")}
        />
      )}
      {route === "detail" && active && (
        <Detail listing={active} onBack={() => setRoute("explore")} onGoPricing={() => setRoute("pricing")} />
      )}
    ...
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-slate-600">
          <button onClick={() => onNavigate("explore")} className="hover:text-slate-900">
            Explorar
          </button>
          <a href="#como-funciona" className="hover:text-slate-900">
            Cómo funciona
          </a>
          <button onClick={() => onNavigate("pricing")} className="hover:text-slate-900">
            Precios
          </button>
        </nav>
        <a className="inline-flex items-center px-4 py-2 rounded-2xl bg-indigo-600 text-white hover:opacity-90" href={WHATSAPP_LINK}>
          Solicitar acceso
        </a>
      </div>
    </header>
...
function Explore({ onOpen, onGoPricing }: { onOpen: (l: Listing) => void; onGoPricing: () => void }) {
  ...
  const filtered = useMemo(() => {
    return DUMMY_LISTINGS.filter(
      (l) => (!district || l.district === district) && (!type || l.type === type) && (!price || l.priceUSD <= price)
    );
  }, [district, type, price]);
      ...
      <div className="grid md:grid-cols-3 gap-6 items-start">
        ...
        <div className="md:col-span-1 rounded-2xl border border-slate-200 bg-white">
          <div className="p-4 border-b border-slate-100">
            <div className="text-sm font-semibold">Filtros (demo)</div>
          </div>
          <div className="p-4 space-y-3">
            ...
            <div>
              ...
              <input
                ...
                className="w-full mt-1 rounded-xl border-slate-200"
              />
            </div>
            <div className="text-xs text-slate-500">* Filtros demo (sin backend)</div>
            <button onClick={onGoPricing} className="w-full rounded-xl border border-slate-200 py-2 hover:bg-slate-50">
              Ver planes
            </button>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              {filtered.map((l) => (
                <div
                  key={l.id}
                  className="rounded-xl overflow-hidden border border-slate-200 bg-white cursor-pointer"
                  onClick={() => onOpen(l)}
                >
              ...
              <div className="h-80 bg-gradient-to-br from-slate-100 to-slate-200 relative">
                {filtered.slice(0, 6).map((l, idx) => (
                  <div
                    key={l.id}
                    className="absolute"
                    style={{ top: `${10 + idx * 8}%`, left: `${15 + ((idx % 3) * 20)}%` }}
                  >
                ...
                <div className="absolute bottom-3 right-3">
                  <button className="rounded-xl text-xs border border-slate-200 bg-white/80 px-3 py-2 hover:bg-white">
                    Buscar en esta área
                  </button>
                </div>
              </div>
...
function Detail({ listing, onBack, onGoPricing }: { listing: Listing; onBack: () => void; onGoPricing: () => void }) {
  return (
    <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <button onClick={onBack} className="text-sm text-slate-600 flex items-center gap-2 mb-4 hover:text-slate-900">
        ← Volver
      </button>
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="mt-3 text-sm text-slate-600">
            {listing.type} — {listing.m2} m² — {listing.district}
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <a href={WHATSAPP_LINK} className="rounded-xl w-full text-center px-4 py-2 bg-indigo-600 text-white hover:opacity-90">
              Contactar
            </a>
            <button onClick={onGoPricing} className="rounded-xl w-full border border-slate-200 px-4 py-2 hover:bg-slate-50">
              Ver planes
            </button>
          </div>
...
function Pricing({ onBack }: { onBack: () => void }) {
  return (
    <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <button onClick={onBack} className="text-sm text-slate-600 flex items-center gap-2 mb-6 hover:text-slate-900">
        ← Volver
      </button>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="text-4xl font-extrabold mt-2">
            S/49<span className="text-base font-semibold text-slate-500"> / mes</span>
          </div>
          ...
          <div className="mt-6">
            <a href={WHATSAPP_LINK} className="rounded-xl w-full text-center px-4 py-2 bg-indigo-600 text-white hover:opacity-90 inline-block">
              Solicitar acceso
            </a>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-indigo-200 bg-white p-6">
          ...
          <div className="text-4xl font-extrabold mt-2">
            + S/24.5<span className="text-base font-semibold text-slate-500"> / mes</span>
          </div>
          <ul className="mt-4 space-y-2 text-slate-700 text-sm">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            ...
            <a href={YAPE_LINK} className="rounded-xl w-full text-center px-4 py-2 border border-slate-200 hover:bg-slate-50">
              Reservar cupo (Yape)
            </a>
          </div>
    ...
    <footer className="py-10 border-t border-slate-100 bg-white mt-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-3 gap-6 items-start">
        ...
        <div>
          ...
          <a href={WHATSAPP_LINK} className="inline-block rounded-2xl px-4 py-2 bg-indigo-600 text-white hover:opacity-90">
            Solicitar acceso
          </a>
      ...
      <div className="mt-8 text-center text-xs text-slate-400">
        &copy; {new Date().getFullYear()} Vistavia. Todos los derechos reservados.
      </div>
    </footer>
  );
}
